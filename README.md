...
| `GET  /api/starred` | ⭐ 즐겨찾기 목록 |
| `POST /api/star` | 즐겨찾기 켜기/끄기 |
...
| **채널활력** | 최근 90일 조회수 중앙값 ÷ 구독자수 | 같은 체급 중앙값의 **절반 이하**면 ⚠️ |
...
침투력과 활력은 **구독자 수에 직접 매인 지표**입니다. 100만 채널의 침투력 0.5 와
1만 채널의 0.5 는 전혀 다른 사건인데 한 줄로 세우면 큰 채널이 늘 아래에 깔립니다.
그래서 이 둘은 **같은 체급 안에서만** 비교합니다.
...
체급이 바꾸는 것은 셋입니다. **숫자 자체는 보정하지 않고 그대로 표시합니다.**
...
| ⚠️ 활력 경고 | **같은 체급 활력 중앙값의 `DEAD_VS_TIER`(0.5) 배 이하** 그리고 **절대 활력도 `DEAD_ABS`(0.05) 미만**일 때만 (AND) |

- **왜 AND 인가** — 체급 상대평가만 쓰면 새 오탐이 생깁니다. 실측 예로 `어른의 지혜` 는
  활력 0.174(구독자의 17% 가 최근 영상을 봅니다 — 절대적으로 건강)인데, `~5만` 체급이
  워낙 활발해서(중앙값 0.79) 절반 기준에 걸립니다. 절대 기준과 묶어 이런 경우를 걸러냅니다.
- **대형 아카이브 채널은 여전히 잡힙니다.** `EBSDocumentary` 는 구독자 543만에 최근 중앙
  조회수 1.8만이라 활력 0.0033 — 체급 문턱(0.0545)과 절대 기준(0.05) 양쪽 모두 크게 밑돕니다.
  체급 보정으로는 사라지지 않는 값이며, 없애려면 채널별 예외 목록이나 "자기 과거 대비"
  방식의 다른 활력 정의가 필요합니다.
- 활력 기준은 화면에 뜬 영상이 아니라 **채널 전체**를 모집단으로 잡습니다(기준이 흔들리지 않게).
- 체급에 채널이 `VITALITY_MIN_CH`(4)개 미만이면 기준을 세우지 않고 **경고를 아예 띄우지 않습니다.**
  근거 없는 ⚠️ 보다 조용한 편이 낫습니다. EBS 같은 대형 소스 채널에 상시로 붙던 오탐이 이렇게 사라집니다.
- 구독자 수를 아직 모르는 채널은 체급이 없어 등급·경고에서 빠지고, 발굴점수에는 중립값(0.5)이 들어갑니다.
...
| `VITALITY_MIN_CH` | 4 | 체급에 채널이 이만큼 있어야 활력 기준을 세운다 |
