...
logs/               수집 실패 로그 (자동 생성 · git 추적 안 함)
```
...
| **침투력** | 조회수 ÷ 구독자수 | 1.0 초과 = 구독자 밖으로 퍼진 영상 |
| **참여율** | (좋아요 + 댓글 × `COMMENT_WEIGHT`) ÷ 조회수 × 100 (%) | 조회수 대비 반응의 양 |
| **발굴점수** | `log(1+배율) × log(1+침투력) × (1 + 참여율 / ENGAGE_DIVISOR)` | 발굴 탭 기본 정렬 |
| **채널활력** | 최근 90일 조회수 중앙값 ÷ 구독자수 | `DEAD_CHANNEL` 미만이면 ⚠️ (죽은 채널의 과거 영광) |
...
| `DEAD_CHANNEL` | 0.05 | 채널활력이 이 아래면 ⚠️ |
