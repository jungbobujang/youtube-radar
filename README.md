...
주요 화면: 💎 발굴 · 🗂 후보함 · 📡 신작 · 📊 주간 · 🔥 HOT · 📈 추적 중 · ⚙️ 감시 관리
...
- **새 광맥 자동 탐사(키워드 검색)는 기본 off** 입니다. 1건당 100유닛이라 가장 비쌉니다.
  `⚙️ 감시 관리` 의 토글로 켜거나, `🔎 지금 탐사` 버튼으로 그때만 돌립니다.
...
| 폭 | 레이아웃 |
|---|---|
| ~640px | 글자·뱃지 축소, 필터 가로 스크롤, 감시 관리 카드형 |
| ~768px | **영상 행 2줄** (1줄 썸네일+제목 / 2줄 지표+버튼), 터치 영역 44px |
| 769px~ | 데스크톱 **1줄** — 썸네일 96px \| 제목·채널·조회·날짜 \| 지표 \| 버튼 |
| ~1179px | 버튼 라벨(`.lbl`)을 떼고 아이콘만 (라벨은 `title` 속성에) |
| 1024px~ | 지표 열 최소 220px — 뱃지가 한 개씩 세로로 흐르는 것을 막음 |

데스크톱 1줄 배치가 무너졌던 적이 있습니다(제목이 `남들에…` 로만 보이고 메타가
세로로 쌓임). 원인은 미디어쿼리가 아니라 **폭 배분**이었습니다 — `.wrap` 이 880px 인데
지표 열(296px 고정) + 버튼 5개가 700px 가까이 먹어 `.meta` 가 36~117px 로 짜부라졌습니다.
지금은 컨테이너를 1240px 로 넓히고, `.tools` 를 `flex: 0 1 auto`(줄어들 수 있게) +
`min-width: auto`(제 내용보다 작아지지는 않게)로 둡니다. 이 둘은 **반대 방향의 같은 사고**라
한쪽만 고치면 제목이 짜부라지거나 버튼이 오른쪽으로 삐져나갑니다.
...
| `GET  /api/hot` | 🔥 HOT 세 칸 (뜨는 주제·추천 채널·해외 광맥) |
...
기준은 `server.js` 의 `AUDITION` 상수에서 조절합니다. 발굴 탭 상단에는
...
감시 채널 안쪽만 보면 이미 아는 광맥만 계속 파게 됩니다. HOT 탭은 바깥을 봅니다.
...
- **탐사**는 `🔥 지금 탐사` 버튼(비용을 버튼에 표시)과 **일요일 저녁 20:00 주 1회**(기본 on,
  `⚙️ 감시 관리` 에서 끔)로 돕니다. 검색 1건당 **100유닛**이라 자동은 주 1회뿐입니다.
...
- ⚙️ 감시 관리의 `🚫 관심없음 목록 (N)` 접이식 칸에서 해제 예정일을 보고 `지금 해제` 할 수 있습니다.
...
- `⛏ 백카탈로그 채굴` 을 누르면 감시에 넣되 **`backlog_only = true`** 라 신작 수집에서
  빠집니다. 캔 영상은 💎 발굴 탭에 그대로 합류하고(배율·지표 계산 동일),
  **💀 휴면 광산** 그룹에 자동 배정됩니다.
- 같은 판정을 **기존 감시망에도** 적용합니다 — 감시 관리와 발굴 채널 필터의 휴면 채널에
  💀/😴 가 붙습니다.
...
- 적용 범위는 💎 발굴 · 📡 신작 · 🔥 HOT 탭입니다. 🗂 후보함과 📊 주간은 목록 성격상
  분포가 의미 없어 숫자만 나옵니다.

> **⭐ 은 모든 탭에서 같게 동작합니다.** 어느 목록에서 눌러도 그 자리에서 사라지고
> 후보함으로 갑니다(낙관적 업데이트 — 실패하면 되돌아옵니다). 각 탭 상단의
