...
- 하루 2회 자동 수집(06:00·21:00 KST) + 필요할 때 `지금 수집`
...
없으면 꺼짐입니다. 별도 테이블 없이 `⚙️ 관리` 화면의 체크박스로 조작합니다.
...
| `channels.list` | 1 | 채널 구독자 갱신(50개당 1), 업로드 재생목록 ID 최초 1회, **채널 추가(핸들)** |
| `search.list` | **100** | 키워드 감시, `유튜브 전체에서 찾기` 버튼, 채널 추가 폴백 |

### 채널 추가 — 받아들이는 입력 형태

`POST /api/watches` 의 채널 입력은 `parseChannelInput()` 이 먼저 해석한다.
**퍼센트 인코딩을 가장 먼저 푸는 것이 핵심이다** — 한글 핸들을 주소창에서 복사하면
`@%EC%B1%85%EA%B3%BC%EC%82%B6` 처럼 붙는데, 이걸 안 풀면 핸들로 인식되지 않는다.

| 입력 | 판정 | 유닛 |
|---|---|---|
| `UCgUaqrLRGLL-dHislR1e2TA` · `youtube.com/channel/UC...` | 채널ID 그대로 | **0** |
| `@책과삶` · `youtube.com/@책과삶/videos` · `@%EC%B1%85%EA%B3%BC%EC%82%B6` | `channels?forHandle` | **1** |
| `youtube.com/c/이름/videos` · `youtube.com/user/이름` · 채널 이름만 | `search` 폴백 | **100** |
| `youtube.com` (알맹이 없음) | 조회 안 함 | 0 |

- 핸들 정규식은 `[\p{L}\p{N}._-]+` (유니코드) 다. 한글·일본어 핸들과 `@수치남-l3z`
  같은 혼합 핸들이 모두 걸리고, `/` 가 문자 클래스에 없으므로 뒤에 붙는
  **`/videos` · `/shorts` · `/featured` · `/streams` 꼬리와 `?si=...` 쿼리는 저절로 잘린다.**
- 이중 인코딩(`%25EC...`)은 변화가 멈출 때까지 최대 3번 푼다. 깨진 시퀀스(`%ZZ`)는
  `decodeURIComponent` 가 던지므로 그때는 원문을 그대로 쓴다.
- 핸들 조회가 비면 `@` 를 떼고 이름 검색까지 한 번 더 간다(개명·옛 채널 대응).

> **고치기 전에는** 핸들 정규식이 ASCII 전용(`[A-Za-z0-9._-]`)이라 **한글 핸들이 전부**
> 100유닛 검색으로 샜다. 특히 스킴 없이 붙여넣은 인코딩 URL 은 검색어가 `youtube.com`
> 으로 잡혀 **엉뚱한 채널이 등록**됐다.
...
| 백카탈로그 | 최대 **260** | `BACKFILL_UNIT_BUDGET` 상한 · 다 캔 채널은 제외 |
| 통계 갱신 | 최대 **20** | `REFRESH.UNIT_BUDGET` 상한 (50개당 1) |
| 재생시간 백필 | 최대 **20** | `DURATION_UNIT_BUDGET` 상한 |
| 채널 정보 갱신 | 채널 50개당 **1** | 하루 1회 (`CHANNEL_TTL_H`) |
| 댓글 수집 | 영상당 **1** | 주 1회 · 회당 최대 100 (`COMMENT_WEEKLY_CAP`) |

**실측(채널 24개 · 절약 모드): 하루 약 700유닛** — 상태바 `📐 예상` 이 현재 설정 기준으로
같은 값을 계산해 보여 줍니다. 백카탈로그가 다 차면 그 단계가 0 이 되어 **하루 200유닛대**로
떨어집니다.

- 키워드 감시 **1개** = 하루 202유닛(2회 기준). 자동 탐사를 켰을 때만 듭니다.
- 위험한 쪽은 채널 수가 아니라 **키워드 개수**입니다. 3개를 넘기지 마세요.
- 같은 안내가 대시보드 `⚙️ 관리` 상단에도 현재 개수와 함께 표시됩니다.

### 절약 모드 (v1.7)

하루 8회 돌던 수집을 **하루 2회(한국시간 06:00 · 21:00)** 로 줄이고, 매번 전부 다시 읽던
통계를 **영상 성격별로 차등** 갱신합니다. `지금 수집` 수동 버튼은 그대로입니다.

| 대상 | 통계 갱신 주기 |
|---|---|
| 게시 30일 이내 영상 | 매 수집마다 |
| ⭐ 후보로 담은 영상 | 하루 1회 |
| 그 밖의 백카탈로그 | 주 1회 (매번 조금씩 돌아가며) |

- 주기 상수는 `server.js` 상단 `COLLECT_CRON` · `REFRESH`(`FRESH_DAYS` / `PICK_HOURS` /
  `BACKLOG_DAYS` / `UNIT_BUDGET` / `CANDIDATES`) 에서 고칩니다.
- **백카탈로그를 다 캔 채널은 순회에서 빠집니다** — 영상 500개 상한에 닿았거나
  업로드 목록을 끝까지 훑은 채널입니다(완료 표시에는 `TODO-SQL.md` 0-E 가 필요).
- **새 광맥 자동 탐사(키워드 검색)는 기본 off** 입니다. 1건당 100유닛이라 가장 비쌉니다.
  `⚙️ 관리` 의 토글로 켜거나, `🔎 지금 탐사` 버튼으로 그때만 돌립니다.
- 재시작할 때마다 수집하지 않습니다. 최근 6시간(`BOOT_COLLECT_GAP_H`) 안에 다른
  인스턴스가 이미 돌았으면 시작 수집을 건너뜁니다(할당량 장부로 판단).
- 상태바에 **`📐 예상 N유닛/일`** 이 뜹니다. 지금 설정 그대로 하루를 돌렸을 때의 추정치이며,
  툴팁에 항목별 내역이 나옵니다.
...
서버는 죽지 않고 다음 cron 에 정상 복귀하며, 화면의 최근 수집 줄에
`할당량 초과로 중단 · 태평양 자정(한국 오후 4~5시경) 리셋 후 재시도` 가 표시됩니다.

**할당량은 이 앱이 아니라 같은 API 키를 쓰는 구글 클라우드 프로젝트 전체 기준**입니다.
Railway·집·학교에서 같은 키를 쓰면 서로의 사용량을 함께 깎아 먹습니다(그래서 `yt_quota_log`
합산이 필요합니다). 수동 요청은 **429** 와 함께 같은 안내를 돌려줍니다.
...
최소 2회 수집이 지나야 유형이 잡힙니다(절약 모드에서는 반나절~하루).
...
예전에는 탭이 7개였고 폰에서 탭 줄이 가로로 스크롤됐습니다. 하는 일을 기준으로 넷으로 접었습니다.

| 탭 | 하는 일 | 흡수한 옛 탭 |
|---|---|---|
| **📥 브리핑** (기본) | 오늘 볼 것만. ① 예심 통과 검토 대기 ② 후보함 이번 주 주목 상위 3 ③ HOT 최근 탐사 3줄 ④ 상태 한 줄 | (신설) |
| **💎 광산** | 캐는 곳. 상단 **갱도 칩**으로 전환 — `💎 묵은 검증`(기본) / `📡 신작` / `🔥 HOT` (+급상승 설정 시 `🔥 급상승`) | 💎 발굴 · 📡 신작 · 🔥 HOT · 🔥 급상승 |
| **⭐ 후보함** | 담은 것 고르기 (변경 없음) | 🗂 후보함 |
| **⚙️ 관리** | 감시 대상·수집 설정 + 접이식 섹션 두 개 | ⚙️ 감시 관리 · 📊 주간 · 📈 추적 중 |

- 각 갱도는 **기존 필터를 그대로** 씁니다 (정렬·기간·형식·그룹·채널·구독자).
- 브리핑의 각 칸에는 `전체 보기 →` 가 있어 해당 탭(HOT은 광산 › HOT 갱도)으로 넘어갑니다.
  볼 게 하나도 없으면 `오늘은 조용해요 ✅` 한 줄만 뜹니다.
- 관리 탭의 **📊 주간 리포트 / 📈 추적 중**은 접혀 있다가 **펼 때 처음 부릅니다**.
  주간 리포트의 "열 때만 계산한다"는 성질이 그대로 유지됩니다.
- 급상승처럼 설정에 따라 생기는 화면은 탭을 늘리지 않고 **갱도 칩**으로 붙습니다
  (탭 줄이 다시 스크롤되기 시작하면 원점이라).

### 뱃지 위계 — 1열과 [⋯] 상세

영상 행의 지표를 두 층으로 갈랐습니다. 훑을 때 필요한 것만 남기고 나머지는 접습니다.

| 층 | 내용 |
|---|---|
| **1열** (항상) | `🅐 민감` · **배율**(등급색) · **참여**(등급색) · **포화도** `✅`/`⚠️` 만 |
| **[⋯] 상세 줄** | 침투 · 일평균/최근7일(에버그린) · 💬 진한반응 · 🗣️ 토론형 · 활력 상세(`⚠️ 쇠락 0.31x`) + 🔎 관련 · 💬 댓글 |

- **등급을 뱃지 바탕색으로** 옮겼습니다. `x3.2` 라는 숫자만으로는 좋은지 모르지만
  초록(Great)/연빨강(Low)이면 훑는 것만으로 걸러집니다. Normal 은 색을 주지 않습니다.
- **포화도는 낱말을 떼고 아이콘만** 남겼습니다 (뜻은 `title`, 개수는 [⋯] 안에).
- **채널 활력(⚠️ 쇠락 / 📈 상승)은 영상 행에서 뺐습니다.** 채널의 성질이지 그 영상의
  성질이 아니라서, 지표 줄에 섞여 있으면 "이 영상이 쇠락했다"로 읽힙니다.
  지금은 **채널명 뒤에 작은 ⚠️/📈** 로만 붙습니다 (마우스를 올리면 숫자가 나옵니다).
- `🅐 민감`은 교사 신분상 걸러야 하는 방화벽이라 1열에 남깁니다.
- **행 액션**도 같이 정리했습니다 — 기본 노출은 `⭐` `💡 아이디어로` `⋯` 셋뿐이고,
  `🔎 관련`·`💬 댓글`은 [⋯] 상세 줄로 내렸습니다. 💬 댓글은 원래대로 후보함 행에서만 켜집니다.

### 📡 신작 소음 차단

신작은 감시 채널이 올린 것을 전부 받아 오므로, 한 채널이 하루에 다섯 개를 올린 날
목록이 그 채널로 덮입니다. 상단 토글 **`[신호만(기본) / 전부]`** 로 가릅니다.

| 모드 | 동작 |
|---|---|
| **신호만** (기본) | 배율 등급 **Low 행 숨김** + **채널당 3건**까지만 펴고 나머지는 그 자리에 `이 채널 N건 더` 로 접음 |
| 전부 | 받은 그대로 |

숨기거나 접은 건수는 목록 위에 `🔇 Low 등급 28건 숨김 · 채널당 3건 초과 37건 접힘` 처럼
항상 적습니다. **조용히 줄이지 않습니다.** 💎 묵은 검증 갱도는 이미 발굴점수순이라 이 체를 걸지 않습니다.

### 폭별 레이아웃

대시보드는 폭에 따라 접힙니다. 실측 결과는 `MOBILE-CHECK.md` 참고.

| 폭 | 레이아웃 |
|---|---|
| ~640px | 글자·뱃지 축소, 필터 가로 스크롤, 관리 탭 카드형, 상세 줄 동작 버튼 전체 폭 |
| ~768px | **영상 행 3줄** (썸네일+제목 / **1열 뱃지** / 버튼), 터치 영역 44px |
| 769px~ | 데스크톱 **1줄** — 썸네일 96px \| 제목·채널·조회·날짜 \| 1열 뱃지 \| 버튼 |
| ~1179px | 버튼 라벨(`.lbl`)을 떼고 아이콘만 (라벨은 `title` 속성에 · 단 [⋯] 상세 줄은 라벨 유지) |
| 1024px~ | 지표 열 최소 220px — 뱃지가 한 개씩 세로로 흐르는 것을 막음 |

**폭 배분 사고 두 건** — 둘 다 미디어쿼리가 아니라 폭 배분 문제였습니다.

1. *데스크톱 1줄이 무너짐* (제목이 `남들에…` 로만 보이고 메타가 세로로 쌓임).
   `.wrap` 이 880px 인데 지표 열(296px 고정) + 버튼 5개가 700px 가까이 먹어
   `.meta` 가 36~117px 로 짜부라졌습니다. 컨테이너를 1240px 로 넓히고 `.tools` 를
   `flex: 0 1 auto`(줄어들 수 있게) + `min-width: auto`(제 내용보다 작아지지는 않게)로 둡니다.
   이 둘은 **반대 방향의 같은 사고**라 한쪽만 고치면 제목이 짜부라지거나 버튼이 삐져나갑니다.
2. *375px 에서 1열 뱃지가 사실상 사라짐* — 뱃지와 버튼이 같은 줄에 있는데 버튼이 전부
   44px 라, 예심 행(승격·킵·제외·💡·⋯)에서는 뱃지에 남는 폭이 **30px 남짓**이었습니다.
   "항상 보이는 열"이 안 보이면 위계를 나눈 뜻이 없으므로, ~768px 에서는
   `.metrics` 에 `flex: 1 1 100%; order: -1` 을 줘 **한 줄을 통째로** 주고 버튼을 다음 줄로 내립니다.
   (실측: 1열 최소 폭 351px / 375px 뷰포트, 가로 overflow 0)

> **`.hide` 함정** — `.hide { display: none }` 은 위쪽에 선언돼 있어, 아래에서
> `display: flex` 를 쓰는 컴포넌트를 만들면 **특정도가 같아 나중 선언이 이깁니다.**
> `.rowdet` 이 이 함정에 빠져 접어 둔 상세 줄이 전부 펴진 채로 나왔습니다.
> 새 컴포넌트에 `display` 를 줄 때는 `.컴포넌트.hide { display: none }` 을 짝으로 답니다.

| 엔드포인트 | 설명 |
|---|---|
| `GET  /api/session` | 쿠키 인증 상태 |
| `POST /api/login` | 접속 코드 확인, 30일 쿠키 발급 |
| `GET  /api/dig` | 💎 광산 › 묵은 검증 — 배율 높은 과거 영상 (기간·그룹·채널·구독자·형식·정렬) |
| `GET  /api/fresh` | 💎 광산 › 신작 — 감시 채널의 최근 30일 영상 (소음 차단은 화면에서) |
| `GET  /api/weekly` | ⚙️ 관리 › 주간 리포트 — 지난 7일 요약 JSON (섹션을 펼 때만 조회, cron 미등록) |
| `GET  /api/picks` | ⭐ 후보함 목록 + 용도별 요약 (`level`, `target` 필터) · 📥 브리핑도 이걸 씁니다 |
...
| `GET  /api/tracking` | ⚙️ 관리 › 추적 중 — 영상 + 증가량 + 유형 뱃지 (섹션을 펼 때만 조회) |
| `GET  /api/hot` | 💎 광산 › HOT 네 칸 (뜨는 주제·추천 채널·해외 광맥·휴면 광산) · 📥 브리핑 요약도 이걸 씁니다 |
| `POST /api/hot/prospect` | 🔥 지금 탐사 (검색 1건당 100유닛) |
| `POST /api/audition` | 🤖 예심 다시 돌리기 (`?dry=1` 이면 세어만 봄) |
...
| `POST /api/watches` | 감시 추가 (채널은 URL/핸들에서 ID 추출 · `group: {name, icon}` 이면 그 그룹에 배정, 없으면 생성) |
...
발굴 목록을 사람이 훑던 1차 스캔을 기계가 먼저 합니다. **수집이 끝날 때마다 자동 실행**되며
DB 만 보므로 할당량을 쓰지 않습니다. 통과분은 후보함 맨 위 **🤖 예심 통과(검토 대기)** 칸에
쌓이고, 각 행의 `⭐ 승격` / `✕ 제외` 만 누르면 됩니다(제외한 것은 다시 올라오지 않습니다).
...
기준은 `server.js` 의 `AUDITION` 상수에서 조절합니다. 📥 브리핑 맨 위에는
`🤖 예심 통과 N건이 검토를 기다리고 있어요` 배지가 뜨고, 누르면 후보함으로 넘어갑니다.
컬럼 추가 SQL 은 `TODO-SQL.md` **0-F** 에 있습니다.

**후보함은 네 칸으로 나뉩니다.**

| 칸 | 무엇 | 행 버튼 |
|---|---|---|
| 🤖 예심 통과 (검토 대기) | 기계가 걸러 놓은 일반 주제 | `⭐ 승격` `🤔 킵` `✕ 제외` |
| 🅐 전용 후보 | 예심은 통과했지만 **민감 주제** — A(부계정)용 | `⭐ 승격` `🤔 킵` `✕ 제외` |
| 🤔 킵 (다시 볼 것) | 지금 결정하기 애매해 미뤄 둔 것 | `⭐ 승격` `↩ 대기로` `✕ 제외` |
| ⭐ 내 후보 | 담은 것 (⭐ 후보 / ❤️ 확정) | `★` `🤍/❤️` |

`✕ 제외` 는 되돌릴 수 없고(다시 예심에 올라오지 않음), `🤔 킵` 은 언제든 되돌립니다.
킵 컬럼 SQL 은 `TODO-SQL.md` **0-G** 입니다.

#### ⚠️🅐 민감 주제 분류

교사 신분이라 **D 채널(본명·얼굴)에 올리기 어려운 주제**를 미리 갈라 둡니다.
걸러 내는 것이 아니라 **어느 채널용인지 표시**하는 장치입니다.

- 판정은 `server.js` 상단 **`SENSITIVE_KEYWORDS`** 배열의 낱말이 제목에 들어 있는지로 합니다
  (정치·선거·시위·전쟁·혐오·젠더·종교·국가감정 등). **이 배열만 고치면 기준이 바뀝니다.**
- 국가 감정은 **이름이 아니라 깎아내리는 말**을 봅니다(`저주받은` `미개한` `열등` `극혐`
  `국뽕` …). 국가 이름을 그대로 넣으면 "중국 경제 분석" 같은 정상 콘텐츠까지 걸립니다.
  대신 **`COUNTRY_RE` + `NEGATIVE_RE` 조합 규칙**이 있어, 제목에 국가명과 부정어가
  **함께** 있으면 `중국+저주` 처럼 표시하며 걸립니다(둘 중 하나만 있으면 안 걸립니다).
- 예심을 통과해도 일반 검토 대기가 아니라 **🅐 전용 후보** 칸으로 갑니다.
- 그 행에는 `⚠️🅐` 뱃지와 **걸린 낱말**이 함께 표시됩니다.
- 담을 때 용도 태그를 **A** 로 잡습니다(채널 그룹 추정보다 우선).
- 발굴·신작·HOT 목록에서도 해당 영상에 작은 **🅐** 가 붙습니다.

> 낱말 일치라 "AI 반도체 **전쟁**" 같은 비유도 걸립니다. 그래도 목록에서 사라지지는 않고
> A 로 분류될 뿐이라, 애매하면 넣어 두는 편이 안전합니다.
...
감시 채널 안쪽만 보면 이미 아는 광맥만 계속 파게 됩니다. 🔥 HOT 갱도는 바깥을 봅니다.
세 칸으로 나뉘고, **첫 칸은 할당량을 쓰지 않습니다**(이미 모은 DB 로 만듭니다).

| 칸 | 무엇 | 비용 |
|---|---|---|
| 🔥 지금 뜨는 주제 | 최근 90일 게시 영상 중 배율·침투 상위 | 0 |
| 📺 추천 채널 | 탐사로 걸린 **감시망 밖 채널**을 채널 단위로 묶음 — `광맥 N개 발견`·구독자·침투력·대표작 2편·`+ 감시 추가` | 탐사 때만 |
| 🌍 해외 광맥 | 영어 검색어로 찾은 아웃라이어 **영상 목록**. **국내에서 몇 곳이 다뤘는지**를 같이 표시 | 탐사 때만 |

**해외는 영상 단위, 채널 추적은 반복 검증 뒤에.** 해외 검색은 한 번에 처음 보는 채널이
수십 개씩 딸려 옵니다. 그중 한 편이 터진 것은 *그 주제*가 터진 것이지 *그 채널*이 좋은 것이
아닙니다. 그래서 두 칸의 문턱을 다르게 둡니다.

| | 📺 추천 채널로 올라오는 조건 | 그 칸에서 할 수 있는 것 |
|---|---|---|
| 국내 채널 | 광맥 **1개** | `+ 감시 추가` |
| 🌍 해외 채널 | 탐사 누적으로 광맥 **`HOT.GLOBAL_PROMOTE`(기본 2)개 이상** | `+ 감시 추가` |
| 🌍 해외 광맥(영상) | — | ⭐ 담기 · 💡 아이디어로 **둘뿐** (감시 추가 버튼 없음) |

- **광맥** = 조회수가 구독자 수를 넘겼거나(`HOT.OUTLIER_REACH`) 조회수 5만 이상
  (`HOT.OUTLIER_VIEWS`)인 영상. 채널 승격과 해외 영상 목록이 **같은 잣대**를 씁니다.
- 추천 채널은 **광맥 수 내림차순**(같으면 침투력순)으로 정렬합니다. 대표작도 광맥부터 보여 줍니다.
- 🌍 해외 채널을 `+ 감시 추가` 하면 국내 채널과 **똑같이** 신작·백카탈로그 수집 대상이 되고,
  **🌍 해외 그룹에 자동 배정**됩니다(그룹이 없으면 그때 만듭니다 — 시드).
  `POST /api/watches` 에 `group: { name, icon }` 을 함께 보내면 되고, 그룹 배정이 실패해도
  감시 추가 자체는 되돌리지 않습니다(응답의 `group_error` 로 알려 줍니다).

- **탐사**는 `🔥 지금 탐사` 버튼(비용을 버튼에 표시)과 **일요일 저녁 20:00 주 1회**(기본 on,
  `⚙️ 관리` 에서 끔)로 돕니다. 검색 1건당 **100유닛**이라 자동은 주 1회뿐입니다.
- 국내 검색어는 **후보함·발굴 상위 제목에서 자동 추출**합니다(이미 감시 중인 채널
  이름은 제외). 해외 검색어는 `HOT_KEYWORD_PAIRS` 상수의 국문↔영문 쌍입니다:
  자기계발→`self improvement` · 뇌과학→`neuroscience` · 습관→`habits` ·
  심리학→`psychology` · 동기부여→`motivation` · 미루기→`procrastination`.
  이 배열만 고치면 탐사 대상이 바뀝니다(앞에서 `HOT.EN_TERMS` 개만 씁니다).
- `국내 N개 채널이 다룸` 은 그 검색어의 **국문 짝**으로 포화도 로직을 그대로 돌린 값입니다.
  국내에서 덜 다뤄진 것이 위로 옵니다.
- 결과는 **롱폼만** 남깁니다(`HOT.FORMATS`). 조회수순 검색은 그대로 두면 해시태그
  쇼츠가 화면을 덮습니다 — 실측에서 상위 30건이 전부 쇼츠였습니다.
- 해외 영상도 ⭐ 로 후보함에 담을 수 있고, 제목 앞에 **🌍** 가 붙습니다.
  `[아이디어로]` 복사는 `[해외] 원제 | URL` 형식으로 나갑니다.
- 발견한 채널의 구독자 수는 `yt_channels` 에 캐시합니다(50개당 1유닛).
  감시 채널이 아니므로 배율(`recent_median_views`)은 계산하지 않습니다.

**1회 탐사 비용** = (국내 `HOT.KR_TERMS` + 해외 `HOT.EN_TERMS`) × 101유닛 + 채널 조회 몇 유닛
+ 휴면 판정(아래) 몇십 유닛. 기본값(3+3)이면 약 **608유닛**입니다.

### 🚫 관심없음 — 추천에서 6개월 밀어내기

추천 채널 카드의 `🚫 관심없음` 을 누르면 그 채널이 **6개월**(`DISMISS_MONTHS`) 동안
📺 추천 채널과 💀 휴면 광산에서 빠집니다. 지우는 게 아니라 `until` 을 적어 두는 쿨다운이라
기한이 지나면 알아서 다시 올라옵니다.

- 누르면 **즉시 화면에서 사라지고**(낙관적) `되돌리기` 토스트가 3초 뜹니다.
  서버가 거절하면 지운 행을 도로 붙입니다.
- **빠진 만큼 차순위가 올라와 목록 길이가 유지됩니다.** 자르기(`slice`) *전에* 걸러내기
  때문입니다 — 따로 채워 넣는 코드도, 추가 API 호출도 없습니다.
  (뒤에서 거르면 20개 목록이 17개로 줄어듭니다. 실측 테스트로 두 순서를 비교해 뒀습니다)
- 같은 채널을 또 누르면 기간을 다시 6개월로 셉니다.
- 만료된 행은 **조회에서 무시**만 하고 실제 삭제는 수집 사이클에서 합니다 —
  읽기만 하는 요청이 쓰기를 하지 않게 하려는 것입니다.
- ⚙️ 관리의 `🚫 관심없음 목록 (N)` 접이식 칸에서 해제 예정일을 보고 `지금 해제` 할 수 있습니다.

**SQL 은 `TODO-SQL.md` 0-I 를 실행해야 합니다.** 실행 전에는 버튼이 503 으로 안내하고
나머지는 그대로 돕니다.

### 💀 휴면 광산 — 멈춘 채널의 검증된 과거

마지막 업로드가 오래됐는데 **히트작이 있는** 채널입니다. 주제는 이미 통했는데 사람이
떠난 광산이라, 그 과거 영상이 통째로 검증된 소재입니다. 신작이 없으니 한 번 캐면 끝이고,
그래서 수집 비용도 1회성입니다. 판정 기준은 `server.js` 의 `DORMANT` 상수입니다.

| 등급 | 조건 |
|---|---|
| 😴 휴면 | 마지막 업로드 **`MIN_DAYS`(90)일 ~ `DEAD_DAYS`(365)일** |
| 💀 완전 휴면 | 마지막 업로드 **1년 이상** |
| 히트작 | 최고 조회수가 **구독자의 `HIT_REACH`(3)배 이상** 또는 **`HIT_VIEWS`(50만) 이상** |

- **판정은 두 단계**입니다. ① DB 에 쌓인 영상의 최신 게시일로 후보를 추리고(할당량 0)
  ② `🔥 지금 탐사` 때 그 후보의 **실제 마지막 업로드**를 확인해 `yt_channels` 에 캐시합니다.
  확인 비용은 **채널당 약 1유닛**(업로드 재생목록 조회 50채널당 1 + `playlistItems` 1건당 1)
  이고, 한 번에 `CHECK_BUDGET`(20)개까지만, 한 채널은 `RECHECK_H`(주 1회)마다 한 번만 묻습니다.
- ①만 거친 값에는 **추정** 표시가 붙습니다. 감시 중인 채널은 업로드 목록을 통째로 훑으므로
  추정도 정확하지만, HOT 으로 발견한 채널은 **조회수순** 검색 결과뿐이라 실제보다 오래돼
  보일 수 있습니다. 그래서 **확인되지 않은 비감시 채널은 목록에 올리지 않습니다**
  (잘못 캐면 유닛만 버립니다).
- `⛏ 백카탈로그 채굴` 을 누르면 감시에 넣되 **`backlog_only = true`** 라 신작 수집에서
  빠집니다. 캔 영상은 💎 묵은 검증 갱도에 그대로 합류하고(배율·지표 계산 동일),
  **💀 휴면 광산** 그룹에 자동 배정됩니다.
- 같은 판정을 **기존 감시망에도** 적용합니다 — ⚙️ 관리와 갱도 채널 필터의 휴면 채널에
  💀/😴 가 붙습니다.

> `yt_watches` 에는 `(type, value)` 유니크 제약이 없습니다. 채널 추가·채굴 모두
> **넣기 전에 직접 중복을 확인**합니다. DB 오류에 기대면 같은 채널이 두 번 들어가
> 수집이 두 배로 돕니다(실측으로 확인).

**SQL 은 `TODO-SQL.md` 0-H 를 실행해야 합니다.** 실행 전에는 판정이 추정으로만 돌고
채굴이 일반 감시 추가로 넘어갑니다(화면이 그 사실을 알려 줍니다).
...
💎 묵은 검증 갱도의 `⬇️ CSV` 는 서버를 다시 부르지 않고 **화면에 그려진 목록 그대로**
...
| **발굴점수** | `log(1+배율) × 침투력_체급백분위 × (1 + 참여율 / ENGAGE_DIVISOR)` | 묵은 검증 갱도 기본 정렬 |
...
> `침투` 뱃지는 [⋯] 상세 줄에 있고, 마우스를 올리면 체급과 그 안에서의 순위가 나옵니다.
...
- 적용 범위는 💎 광산의 세 갱도(묵은 검증·신작·HOT)와 📥 브리핑의 예심 행입니다.
  ⭐ 후보함과 📊 주간 리포트는 목록 성격상 분포가 의미 없어 숫자만 나옵니다.
- 등급은 **뱃지 바탕색**으로도 나타납니다 (Great 초록 / Low 연빨강 / Normal 무색).
  📡 신작의 `신호만` 모드가 감추는 것이 바로 이 **배율 Low** 행입니다.

> **⭐ 은 모든 목록에서 같게 동작합니다.** 어디서 눌러도 그 자리에서 사라지고
> 후보함으로 갑니다(낙관적 업데이트 — 실패하면 되돌아옵니다). 목록 상단의
> `⭐ 포함 보기` 를 켜면 담아 둔 것도 함께 보입니다. **후보함이 유일한 별 모음입니다.**
...
⚙️ 관리 탭의 접이식 섹션을 **펼 때만** 계산하고, DB 만 읽으므로 할당량을 쓰지 않습니다.
...
채널을 주제별로 묶어 💎 묵은 검증·📡 신작 갱도에서 한 번에 걸러 보기 위한 장치입니다.
...
- 어느 그룹에도 안 든 채널은 `⚙️ 관리` 의 **📂 미분류** 섹션에 모입니다.
...
`⚙️ 관리` 의 채널 줄에 `주간 구독 +1.2천` 칩이 붙습니다.
...
💎 묵은 검증·📡 신작 갱도의 기본값은 **롱폼**입니다. 재생시간이 아직 없는(`null`) 영상은
...
> 묵은 검증 갱도에서는 대부분 `측정 중` 으로 보입니다. 신작 갱도는 정상 동작합니다.
